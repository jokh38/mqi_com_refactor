...
def update_case_status_from_beams(case_id: str, case_repo: CaseRepository):
    ...
    if failed_beams > 0:
        ...
        case_repo.logger.info(f"Case '{case_id}' has failed beams. Marking case as FAILED.")
        case_repo.update_case_status(case_id, CaseStatus.FAILED, error_message=f"{failed_beams} beam(s) failed.")
    elif completed_beams == total_beams:
        # If all beams are completed, the case is completed.
        case_repo.logger.info(f"All {total_beams} beams for case '{case_id}' are complete. Marking case as COMPLETED.")
