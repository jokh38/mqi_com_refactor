...
class CaseRepository(BaseRepository):
    ...
    def add_case(self, case_id: str, case_path: Path) -> None:
        ...
        self._execute_query(
            query, 
            (case_id, str(case_path), CaseStatus.PENDING.value, 0.0)
        )
        
        self.logger.info("Case added successfully", {
            "case_id": case_id,
            "case_path": str(case_path),
            "status": CaseStatus.PENDING.value
        })

    def update_case_status(self, case_id: str, status: CaseStatus, progress: float = None, 
                          error_message: str = None) -> None:
        """
        Updates the status and progress of a case.
        
        FROM: Migrated from the `update_case_status` method in `database_handler.py`.
        ...
        """
        self._log_operation("update_case_status", case_id, 
    ...
    def get_case(self, case_id: str) -> Optional[CaseData]:
        ...
        query = """
            SELECT case_id, case_path, status, progress, created_at, updated_at, 
                   error_message, assigned_gpu
            FROM cases 
            WHERE case_id = ?
        """
    ...
    def record_workflow_step(self, case_id: str, step: WorkflowStep, status: str, 
        ...
        """
        self._log_operation("record_workflow_step", case_id, 
                          step=step.value, status=status)
        ...
        self._execute_query(
            query,
            (case_id, step.value, status, error_message, metadata_json)
        )
    ...
    def get_workflow_steps(self, case_id: str) -> List[WorkflowStepRecord]:
        ...
        query = """
            SELECT case_id, step, started_at, completed_at, status, error_message, metadata
            FROM workflow_steps
            WHERE case_id = ?
    ...
    def assign_gpu_to_case(self, case_id: str, gpu_uuid: str) -> None:
        ...
        query = "UPDATE cases SET assigned_gpu = ?, updated_at = CURRENT_TIMESTAMP WHERE case_id = ?"
